    ...
    <div className='h-screen max-w-screen bg-black overflow-hidden'>
      <div className='flex flex-row h-full'>
        ...
        <div className='flex flex-col flex-1 h-full'>
          <div className='sticky top-0 z-10 w-full flex flex-row justify-between h-18 border border-b flex gap-2 pt-5 pb-6 items-center text-xl border-neutral-500/30 bg-black'>
          <div className='flex gap-2 items-center'>
            <span className='text-neutral-500 ml-5'>Learning Partner</span>
          ...
          <div className='flex gap-3 items-center'>
            <input 
              ...
              className='px-3 py-1 bg-black border border-neutral-500/30 rounded-lg text-white text-sm focus:outline-none focus:border-blue-500'
            />
            <select
              value={filters.tag}
            ...
            >
              ...
              <div className='flex flex-row gap-3 items-center'>
                <button onClick={() => setFindopen(!findopen)} className='flex flex-row items-center px-5 py-1 gap-2 font-semibold border border-[#0340aa] shadow-xl bg-[#0340aa] rounded-xl border-1'>
                    Find 
                </button>

                <button 
                  onClick={() => setOpenai(!openai)}
                  className='flex flex-row items-center px-4 py-1 gap-2 font-semibold border border-[#0340aa] rounded-xl border-1'
                >
                    Find With AI <RiGeminiFill className='text-[#0340aa]'/>
                </button>
          </div>
          </div>
          ...
          <div className='flex-1 p-2 overflow-y-auto' style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
            ...
            {openai && (
              <div 
                className='fixed top-20 right-6 w-80 z-50 rounded-2xl shadow-2xl flex flex-col h-[28rem] bg-black border border-neutral-500/30 backdrop-blur-xl'
                suppressHydrationWarning
              >
                <div className='flex items-center justify-between p-4 border-b border-neutral-500/30'>
                  <div className='flex items-center gap-3'>
                    <div className='w-3 h-3 bg-green-500 rounded-full animate-pulse'></div>
                    <h3 className='text-white font-semibold'>Find with PlusAI </h3>
                  </div>
                  <button 
                ...
                <div className='flex-1 p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-600 scrollbar-track-transparent'>
                  {messages.length === 0 && (
                    <div className='text-center text-neutral-400 mt-8'>
                      <RiGeminiFill className='text-4xl mx-auto mb-3 text-blue-500' />
                      <p className='text-sm'>Ask me to help you find the perfect learning partner!</p>
                    </div>
                  )}
                  {messages.map((msg: any, index: any) => (
                    <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end mb-4' : 'justify-start mb-4'}`}>
                ...
                <div className='p-4 border-t border-neutral-700/50'>
                  <div className='relative flex items-center bg-neutral-800/50 rounded-xl border border-neutral-600/50 focus-within:border-blue-500/50 transition-colors'>
                    <input 
                      ...
                      className='flex-1 bg-transparent text-white px-4 py-3 outline-none  placeholder-neutral-400'
                    />
                    <button
                      onClick={() => sendMessageToAI(input)}
                      disabled={!input.trim()}
                      className='p-2 m-2 bg-blue-600 hover:bg-blue-700 disabled:bg-neutral-600 disabled:cursor-not-allowed text-white rounded-lg transition-colors'
                    >
